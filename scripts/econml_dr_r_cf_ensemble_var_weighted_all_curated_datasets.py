...
def preprocess_covariates(data):
    ...
    ct = ColumnTransformer([("num", "passthrough", num), ("cat", OneHotEncoder(handle_unknown="ignore"), cat)])
...
def dr_cate(X, y, T, mu_hat, prop):
    ...
    for t in range(1, 5):
        ...
        pseudo = treat * (y - mu_hat) / p_t - control * (y - mu_hat) / p_0
...
def forest_cate(X, y, T):
    ...
    for t in range(1, 5):
        ...
        cf = CausalForestDML(n_estimators=400, min_samples_leaf=10, discrete_treatment=True)
...
def build_iCATE(data, tau):
    ...
    for i, t in enumerate(treatments):
        ...
        rows.append(
            pd.DataFrame(
                {"ID": ids, "z": t, "Estimate": est, "L95": est - 1.96 * se, "U95": est + 1.96 * se, "x": subgroup}
            )
        )
...
def save_outputs(dataID, iCATE_df, data):
    ...
    for t in treatments:
        ...
        s_rows.append({"z": t, "Estimate": df.Estimate.mean(), "L95": df.L95.mean(), "U95": df.U95.mean()})
    ...
    for t, x_val in product(treatments, subgroups):
        ...
        df = iCATE_df[(iCATE_df.z == t) & (iCATE_df.x == x_val)]

        sub_rows.append(
            {"z": t, "x": x_val, "Estimate": df.Estimate.mean(), "L95": df.L95.mean(), "U95": df.U95.mean()}
        )
    ...
    pd.DataFrame(rows).to_csv(os.path.join(OUTPUT_DIR, best_sub_file), index=False)
...
def process_dataset(file):
    ...
    tau = ensemble_tau([dr_tau, r_tau, cf_tau], [dr_var.mean(), r_var.mean(), cf_var.mean()])
