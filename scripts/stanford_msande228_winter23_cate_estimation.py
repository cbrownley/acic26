...
def get_data():
    ...
    X.columns = (
        X.columns.str.replace("cat__", "", regex=False)
...
if groups is None:
    X, Xval, D, Dval, y, yval = train_test_split(X, D, y, train_size=0.6, shuffle=True, random_state=123)
    Xval, Xtest, Dval, Dtest, yval, ytest = train_test_split(
        Xval, Dval, yval, train_size=0.5, shuffle=True, random_state=123
    )
    groupsval, groupstest = None, None
else:
    train, val = next(GroupShuffleSplit(n_splits=2, train_size=0.6, random_state=123).split(X, y, groups=groups))
    X, Xval, D, Dval, y, yval = X.iloc[train], X.iloc[val], D[train], D[val], y[train], y[val]
    groups, groupsval = groups[train], groups[val]

    val, test = next(
        GroupShuffleSplit(n_splits=2, train_size=0.5, random_state=123).split(Xval, yval, groups=groupsval)
    )
    Xval, Xtest, Dval, Dtest, yval, ytest = (
        Xval.iloc[val],
        Xval.iloc[test],
        Dval[val],
        Dval[test],
...
if binary_y:
    ...
    model_y = auto_clf(X, y, n_splits=n_splits, split_type=split_type, verbose=verbose, time_budget=time_budget)
    model_reg_zero = auto_clf(
        X[D == 0],
        y[D == 0],
        groups=groups,
        n_splits=n_splits,
    ...
    model_y = auto_reg(
        X, y, groups=groups, n_splits=n_splits, split_type=split_type, verbose=verbose, time_budget=time_budget
    )
...
model_t = auto_clf(
    X, D, groups=groups, n_splits=n_splits, split_type=split_type, verbose=verbose, time_budget=time_budget
)
...
if groups is None:
    ...
    score_reg = np.mean(
        cross_val_score(model_reg_zero(), X[D == 0], y[D == 0], groups=groups[D == 0], cv=cv, scoring="r2")
    )
    print(f"model_reg_zero: {score_reg:.3f}")
    score_reg = np.mean(
        cross_val_score(model_reg_one(), X[D == 1], y[D == 1], groups=groups[D == 1], cv=cv, scoring="r2")
    )
    print(f"model_reg_one: {score_reg:.3f}")
...
dr_preds += (y - reg_preds_t) * (D - prop_preds) / np.clip(prop_preds * (1 - prop_preds), 0.09, np.inf)
...
model_final_fn = lambda Z, y: auto_reg(
    Z, y, groups=groups, n_splits=n_splits, split_type=split_type, verbose=verbose, time_budget=time_budget
)
...
yres = y - res_preds
Dres = D - prop_preds
Dres = np.clip(Dres, 0.001, np.inf) * (Dres >= 0) + np.clip(Dres, -np.inf, -0.001) * (Dres < 0)

rlearner_fn = auto_weighted_reg(
    Z, yres / Dres, sample_weight=Dres**2, groups=groups, n_splits=n_splits, verbose=verbose, time_budget=time_budget
)
...
drlearner_const = make_pipeline(
    PolynomialFeatures(degree=0, include_bias=True), LinearRegression(fit_intercept=False)
).fit(Z, dr_preds)
...
def calculate_dr_outcomes(Xtrain, Dtrain, ytrain, groupstrain, Xval, Dval, yval, groupsval):
    ...
    dr = reg_one_preds_t - reg_zero_preds_t
    reisz = (Dval - prop_preds) / np.clip(prop_preds * (1 - prop_preds), 0.09, np.inf)
...
print(
    f"AUTOC: {autoc:.4f}, s.e.: {autoc_stderr:.4f}, "
...
print(
    f"QINI: {qini:.4f}, s.e.: {qini_stderr:.4f}, "
...
Dres = D - prop_preds
cf = CausalForest(
    4000, criterion="het", max_depth=5, max_samples=0.4, min_samples_leaf=50, min_weight_fraction_leaf=0.0
)
cf.fit(Z, Dres, yres)
...
drrf = RegressionForest(4000, max_depth=5, max_samples=0.4, min_samples_leaf=50, min_weight_fraction_leaf=0.0)
drrf.fit(Z, dr_preds)
